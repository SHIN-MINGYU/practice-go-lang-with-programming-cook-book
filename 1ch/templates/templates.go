...
// RunTemplate function show the exam to initailize templates and variable templates helper functions
func RunTemplate() error {
	...
	// can connect this too
	t := template.New("example")
	t = t.Funcs(funcmap)

	// For reacting to error, 'Must' can use too in this sentence
	...
	// For showing block_example, Atfer copy first template, create another template what parse secondTemplate
	t2, err := t.Clone();
	if err!= nil {
        return err
    }
	...
	// write template in stdout and fill data
